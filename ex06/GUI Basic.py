...
def detect_yolo(frame):
    ...
    if vomit_found:
                ...
                if vomit_count == 10:  # ถ้าพบรถครบ 10 ครั้ง
    ...
    for face_encoding in face_encodings:
        ...
        if name == "Unknown":
            ...
            if known_frame_count >= 10:  
                if current_time - last_known_notified_time > 600:
...
def show_frame_b(label_b, detection_mode, url):
    ...
    if url:
        if running_b:
            ...
            # ลดบัฟเฟอร์ของ OpenCV เพื่อป้องกันการเก็บเฟรมเก่า
...
def start(): 
    ...
    toggle_a_button = ctk.CTkButton(frame_a, text="เปิดกล้อง 1",command=toggle_camera_a)
    toggle_a_button.pack(side="bottom")
    ...
    toggle_b_button = ctk.CTkButton(frame_b, text="เปิดกล้อง 2",command=toggle_camera_b)
    toggle_b_button.pack(side="bottom")

    menu_frame = ctk.CTkFrame(Start_window, height=100)
    menu_frame.pack(pady=5, fill="y", side="top", padx=10)
 
    # Back button
    button1 = ctk.CTkButton(menu_frame, text="ย้อนกลับ", corner_radius=5, width=15,command=close_window)
    button1.pack(side="left", padx=10, pady=10)

    # Settings button
    button2 = ctk.CTkButton(menu_frame, text="ตั้งค่า", corner_radius=5, width=15, command=go_sitting)
    button2.pack(side="left", padx=10, pady=10)

    # Face Recognition button
    button3 = ctk.CTkButton(menu_frame, text="Face Recognition", corner_radius=5, width=20,command=go_Face_Recognition)
    ...
    for mode in modes:
        rb = ctk.CTkRadioButton(radio_frame, text=mode, variable=detection_mode, value=mode, corner_radius=5, width=20)
...
def start_threads(label_a, label_b, detection_mode, url):
    ...
    # Start Camera A on a separate thread
    if url:
        running_b = True
        cap_b = cv2.VideoCapture(url)  # Use URL for camera B
        thread_b = threading.Thread(target=show_frame_b, args=(label_b, detection_mode, url))
        thread_b.start()
    else:
    ...
    running_a = True
    cap_a = cv2.VideoCapture(0)  # Camera A uses index 0
    thread_a = threading.Thread(target=show_frame_a, args=(label_a, detection_mode))
    thread_a.start()
    # Start Camera B only if the URL is provided
...
def setting():
    ...
    exit_sitting_Button = ctk.CTkButton(navigation_frame, text="Exit", fg_color="red",command=exit_sitting, hover_color="#FF9999",text_color="black")
...
def Main_window():
    ...
    logo_KMITL_image = ctk.CTkImage(Image.open(os.path.join(image_path, "KMITL-Photoroom.png")), size=(130, 130))
    logo_RIE_image = ctk.CTkImage(Image.open(os.path.join(image_path, "RIE-Photoroom.png")), size=(130, 130))
    logo_BG_image = ctk.CTkImage(Image.open(os.path.join(image_path, "bg_gradient.jpg")), size=(1200, 1200))
    ...
    # Menu frame
    menu_frame = ctk.CTkFrame(root,corner_radius=30)
    menu_frame.pack(pady=(10,10))  # Use sticky to stretch across the screen

    logo_label = ctk.CTkLabel(menu_frame, text="ระบบความปลอดภัย", font=ctk.CTkFont(size=30, weight="bold"))
    logo_label.pack(padx=10, pady=20)

    start_button = ctk.CTkButton(menu_frame, text="Start",command=start, fg_color="#3fd956", text_color="black", hover_color="#46b842")
    start_button.pack(padx=20, pady=5)

    stop_button = ctk.CTkButton(menu_frame,command=face_recording, text="Face Recognition")
    stop_button.pack(padx=20, pady=5)

    Setting_button = ctk.CTkButton(menu_frame,command=setting, text="Setting", fg_color="#f7eb0a", text_color="black", hover_color="#b8a042")
    Setting_button.pack(padx=20, pady=5)

    Credit_button = ctk.CTkButton(menu_frame,command=credit, text="Credit", fg_color="black", text_color="white", hover_color="darkgray")
    Credit_button.pack(padx=20, pady=5)

    Exit_button = ctk.CTkButton(menu_frame,command=exit_program, text="Exit", fg_color="#FF9999", hover_color="red")
